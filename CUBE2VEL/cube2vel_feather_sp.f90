...
!
SUBROUTINE cube2vel_feather_sp1(vx0,vy0,nx,ny,nz,errx0,erry0,weightx0,weighty0, &
...
REAL*4 weightx(nx,ny,5*nz)
REAL*4 weighty(nx,ny,5*nz)
INTEGER*2 nn(nx,ny)
INTEGER*2 nn2(nx,ny)
INTEGER*2 w(nz)
INTEGER*2 i,j,k,k0
...
weightx(1:nx,1:ny,1:nz) = weightx0
...
weighty(1:nx,1:ny,1:nz) = weighty0
...
DO i = 1, nx
  ...
  ! WEIGHTED STD X
    ...
    !IF ((i .EQ. 62) .AND. (j .EQ. 81)) THEN
    !    print *,velox(i,j),veloy(i,j),vtmpx,vtmpy
    !    print *,vx(i,j,1:nz2)
    !    print *,''
    !    print *,vy(i,j,1:nz2)
    !ENDIF

    IF ( (cnt .GT. 3) .AND. (velox(i,j) .NE. 0) .AND. (veloy(i,j) .NE. 0) ) THEN

        ! REMOVE VALUES MORE THAN 1 STD AWAY FOR MEDIAN
	    DO k = 1, nz2
	        IF (vx(i,j,k) .NE. 0) THEN
	            IF ( (ABS(vx(i,j,k)-velox(i,j)) .GT. 1*vtmpx ) .OR. &
                (ABS(vy(i,j,k)-veloy(i,j)) .GT. 1*vtmpy ) ) THEN
	                vx(i,j,k)=0
	                vy(i,j,k)=0
                    weightx(i,j,k)=0
                    weighty(i,j,k)=0
    ...
    !    print *,vx(i,j,1:nz2)
    !ENDIF
    tmpx = vx(i,j,1:nz2)*weightx(i,j,1:nz2)
    tmpy = vy(i,j,1:nz2)*weighty(i,j,1:nz2)
    cnt=COUNT( (tmpx .ne. 0) .and. (tmpy .ne. 0))
    mask = ( (tmpx .ne. 0) .and. (tmpy .ne. 0))
    nn2(i,j)=cnt
    ! WEIGHTED MEAN X
    IF (cnt .NE. 0) THEN
        

        w_vx(i,j) = sum( tmpx , mask = mask ) / sum( weightx(i,j,1:nz2) , mask = mask )

        IF (cnt .GE. 2) THEN
            stdx2(i,j) = sqrt( cnt * sum( weightx(i,j,1:nz2) * &
            (vx(i,j,1:nz2) - w_vx(i,j))**2 , mask = mask) / &
            sum( weightx(i,j,1:nz2) , mask = mask ) / (cnt-1) )
        ELSE
            stdx2(i,j) = stdx(i,j)
        ENDIF
        ...
        ! WEIGHTED MEAN Y
        w_vy(i,j) = sum( tmpy , mask = mask ) / sum( weighty(i,j,1:nz2), mask = mask )
        IF (cnt .GE. 2) THEN
            stdy2 (i,j) = sqrt( cnt * sum( weighty(i,j,1:nz2) * &
                (vy(i,j,1:nz2) - w_vy(i,j))**2 , mask = mask ) / &
                sum( weighty(i,j,1:nz2) , mask = mask) / (cnt-1) )
        ELSE
            stdy2(i,j) = stdy(i,j)
        ENDIF
    ELSE
