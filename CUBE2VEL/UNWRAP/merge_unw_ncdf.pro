pro merge_unw_ncdf,display=display,nthreads=nthreads 

path='/u/pennell-z0/eric/ANTARCTICA/'
...
SHMMap, 'shm_errx_final'+unik,/float,Dimension=[geo0.npix,geo0.nrec]
SHMMap, 'shm_erry_final'+unik,/float,Dimension=[geo0.npix,geo0.nrec]
SHMMap, 'shm_cnt_final'+unik,/long,Dimension=[geo0.npix,geo0.nrec]
SHMMap, 'shm_cnt_temp'+unik,/float,Dimension=[geo0.npix,geo0.nrec]
...
errx_final = SHMVAR('shm_errx_final'+unik)
erry_final = SHMVAR('shm_erry_final'+unik)
cnt_final = SHMVAR('shm_cnt_final'+unik)
cnt_temp = SHMVAR('shm_cnt_temp'+unik)
...
FOR i=0,nthreads-1 DO BEGIN
    bridge = (i eq 0)? create_struct('n'+string(i,FORmat='(I2)'), Obj_New('IDL_IDLBridge')) : $
        create_struct('n'+string(i,FORmat='(I2)'), Obj_New('IDL_IDLBridge',output=''),bridge) ;;; add output='' FOR debug
ENDFOR
...
FOR ithread=0,nthreads-1 DO BEGIN
    ...
    bridge.(ithread)->EXECUTE,"SHMMap, 'shm_errx_final"+unik+"',/float,Dimension=[geo0.npix,geo0.nrec]"
    bridge.(ithread)->EXECUTE,"SHMMap, 'shm_erry_final"+unik+"',/float,Dimension=[geo0.npix,geo0.nrec]"
    bridge.(ithread)->EXECUTE,"SHMMap, 'shm_cnt_final"+unik+"',/long,Dimension=[geo0.npix,geo0.nrec]"
    bridge.(ithread)->EXECUTE,"SHMMap, 'shm_cnt_temp"+unik+"',/float,Dimension=[geo0.npix,geo0.nrec]"
    ...
    bridge.(ithread)->EXECUTE,"errx_final = SHMVAR('shm_errx_final"+unik+"')"
    bridge.(ithread)->EXECUTE,"erry_final = SHMVAR('shm_erry_final"+unik+"')"
    bridge.(ithread)->EXECUTE,"cnt_final = SHMVAR('shm_cnt_final"+unik+"')"
    bridge.(ithread)->EXECUTE,"cnt_temp = SHMVAR('shm_cnt_temp"+unik+"')"
...
;07595_y07840
;cubes/c_x07840_y09800_post0450_yr*.sav
;3675    4900
FOR i=0,geo0.npix-1,nrec-5 DO BEGIN
                ...
                FOR ith = 0, nthreads-1 DO BEGIN
                    ...
                    IF bridge.(ith)->status() eq 2 or bridge.(ith)->status() eq 0 THEN BEGIN
                        ...
                        IF tmp ne 0 then BEGIN
                            ...
                            bridge.(ith)->execute,'errx_final[i,j] = result.errx[0:(npix-1L)<(geo0.npix-1L-i),0:(nrec-1L)<(geo0.nrec-1L-j)]'
                            bridge.(ith)->execute,'erry_final[i,j] = result.erry[0:(npix-1L)<(geo0.npix-1L-i),0:(nrec-1L)<(geo0.nrec-1L-j)]'
        ...
        ;result=unw2vel_feather_sp_fortran(i,j,geo0,/verbose)

        bridge.(ithread)->execute,'result=unw2vel_feather_sp_fortran(i,j,geo0)',/NOWAIT
...
WHILE NOT FINISH DO BEGIN ; {{{
    ...
    FOR ith = 0, nthreads-1 DO BEGIN
        ...
        IF bridge.(ith)->status() eq 2 or bridge.(ith)->status() eq 0 THEN BEGIN
            ...
            IF tmp ne 0 then BEGIN ;; {{{
                bridge.(ith)->execute,'vx_final[i,j] = result.vx[2:(npix-3L)<(geo0.npix-1L-i),2:(nrec-3L)<(geo0.nrec-1L-j)]'
                bridge.(ith)->execute,'vy_final[i,j] = result.vy[2:(npix-3L)<(geo0.npix-1L-i),2:(nrec-3L)<(geo0.nrec-1L-j)]'
                bridge.(ith)->execute,'stdx_final[i,j] = result.stdx[2:(npix-3L)<(geo0.npix-1L-i),2:(nrec-3L)<(geo0.nrec-1L-j)]'
                bridge.(ith)->execute,'stdy_final[i,j] = result.stdy[2:(npix-3L)<(geo0.npix-1L-i),2:(nrec-3L)<(geo0.nrec-1L-j)]'
                bridge.(ith)->execute,'cnt_final[i,j]  = result.cnt[2:(npix-3L)<(geo0.npix-1L-i),2:(nrec-3L)<(geo0.nrec-1L-j)]'
                bridge.(ith)->execute,'result=0'
                bridge.(ith)->execute,'delvar,result'
                bridge.(ith)->execute,'delvar,i'
                bridge.(ith)->execute,'delvar,j'
        ...
        ENDIF
        WAIT,0.05
        IF TOTAL(Bridges_RUNNING) eq 0 THEN FINISH=1
    ENDFOR
ENDWHILE ; }}}
...
long_name = ['Ice velocity in x direction',$
    ...
    'Err velocity in x direction',$
    'Err velocity in y direction',$
    'Cnt velocity']
...
units = ['meter/year','meter/year','meter/year','meter/year','meter/year','meter/year','Count']
UseFillValue = [1,1,1,1,1,1,0]

save_ncdf_ps,'vel_unwrap.nc',geo0,$
    _EXTRA = {  vx:float(vx_final),$
    vy:float(vy_final),$
    stdx:float(stdx_final),$
    stdy:float(stdy_final),$
    errx:float(errx_final),$
    erry:float(erry_final),$
    cnt:cnt_final},$
    long_name=long_name,$
    UseFillValue=UseFillValue,$
...
;; }}}
 ...
 SHMUNMAP, 'shm_vx_final'+unik
 SHMUNMAP, 'shm_vy_final'+unik
 SHMUNMAP, 'shm_stdx_final'+unik
 SHMUNMAP, 'shm_stdy_final'+unik
 SHMUNMAP, 'shm_errx_final'+unik
 SHMUNMAP, 'shm_erry_final'+unik
 SHMUNMAP, 'shm_cnt_final'+unik

 FOR i=0,nthreads-1 DO OBJ_DESTROY, bridge.(i)
