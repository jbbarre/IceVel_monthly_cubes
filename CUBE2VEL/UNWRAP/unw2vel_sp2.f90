...
REAL*4 INVABC(2,2)
REAL*4 vx(10000),vy(10000),angle(10000)
INTEGER*2 nn(nx,ny)
...
DO i = 1, nx
    ...
    DO j = 1, ny
                ...
                DO l = k+1,nz 
                    IF ((v(i,j,l) .NE. 0) .AND. (NUM .LE. 10000)) THEN ! {{{
                        ...
                        IF (ABS(min(360.*dtor - abs(alpha - beta),abs(alpha -beta))) .GE. 10.*dtor) THEN ! {{{
                            ...
                            angle(num) = min(360.*dtor - abs(alpha),abs(alpha))
        ...
        velox(i,j) = sum( VX*angle*angle , mask =  ((vx .ne. 0) .and. (vy .ne. 0))) / &
        sum( angle*angle , mask =  ((vx .ne. 0) .and. (vy .ne. 0)))
        
        veloy(i,j) = sum( VY*angle*angle , mask =  ((vx .ne. 0) .and. (vy .ne. 0))) / &
        sum( angle*angle , mask =  ((vx .ne. 0) .and. (vy .ne. 0)))
        ...
        IF (cnt .GE. 3) THEN
            ...
            DO k=1,10000
                IF (ABS(vx(k)-velox(i,j)) .GE. stdx(i,j)) .AND. (ABS(vy(k)-veloy(i,j)) .GE. stdy(i,j)) THEN 
                    vx(k)=0
                    vy(k)=0
                ENDIF
            ENDDO
