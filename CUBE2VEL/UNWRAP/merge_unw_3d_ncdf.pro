pro merge_unw_3d_ncdf,display=display 
...
SHMMap, 'shm_vz_final'+unik,/float,Dimension=[geo0.npix,geo0.nrec]
...
SHMMap, 'shm_stdz_final'+unik,/float,Dimension=[geo0.npix,geo0.nrec]
...
FOR ithread=0,nthreads-1 DO BEGIN
    ...
    bridge.(ithread)->EXECUTE,"SHMMap, 'shm_vz_final"+unik+"',/float,Dimension=[geo0.npix,geo0.nrec]"
    ...
    bridge.(ithread)->EXECUTE,"SHMMap, 'shm_stdz_final"+unik+"',/float,Dimension=[geo0.npix,geo0.nrec]"
...
if keyword_set(display) then winjer,geo0.npix/25.,geo0.nrec/25.,title=pwd+' vz'
;07595_y07840
;04900_y03675
FOR i=0,geo0.npix-1,nrec-5 DO BEGIN
    FOR j=0,geo0.nrec-1,npix-5 DO BEGIN
                ...
                FOR ith = 0, nthreads-1 DO BEGIN
                    ...
                    IF bridge.(ith)->status() eq 2 or bridge.(ith)->status() eq 0 THEN BEGIN
                        ...
                        IF tmp ne 0 then BEGIN
                            ...
                            bridge.(ith)->execute,'vz_final[i,j] = result.vz[0:(npix-1L)<(geo0.npix-1L-i),0:(nrec-1L)<(geo0.nrec-1L-j)]'
                            ...
                            ;bridge.(ith)->execute,'stdz_final[i,j] = result.stdz[0:(npix-1L)<(geo0.npix-1L-i),0:(nrec-1L)<(geo0.nrec-1L-j)]'
        ...
        bridge.(ithread)->execute,'result=unw2vel_3d_sp_fortran(i,j,geo0,sensors=sensors)',/NOWAIT
        ;result=unw2vel_3d_sp_fortran(i,j,geo0,sensors=sensors,/verb)
        ;stop
        ...
        IF j eq 0 and keyword_set(display) THEN BEGIN
            ...
            wset,33
            tvscl,congrid( vz_final, geo0.npix/25., geo0.nrec/25.) > (-50) < 50 ,/nan
        ENDIF
    ENDFOR
ENDFOR
...
long_name = ['Ice velocity in x direction',$
    ...
    'Ice velocity in z direction']

standard_name = ['land_ice_x_velocity',$
            'land_ice_y_velocity',$
            'land_ice_z_velocity']

units = ['meter/year','meter/year','meter/year']

save_ncdf_ps_south,'unwrap_3d.nc',geo0,vx=vx_final,vy=vy_final,vz=vz_final, long_name=long_name,$
    standard_name=standard_name,units=units


   write_jpeg,'unw_3d_final.jpg',bytscl((abs(congrid(complex(vx_final,vy_final),1244,1244))<3000)^0.35)
stop
end
